...
RFC-0009 specifies Argon2id as the password KDF. ADR-005 requires no new encryption profiles until existing profiles are frozen and covered by cross-language test vectors. However, ADR-005 lists `kdna-password-protected-v1` as "Implemented" in JS Core — that implementation uses `@noble/hashes` (Argon2id), which introduces a native dependency risk for CI and end-user installs.

## Decision

**Add a scrypt-based password profile as the B2 v0.1 write profile.** The profile ID is `kdna-password-protected-v1-scrypt`.
...
3. **Migration path is clean.** `kdna-password-protected-v1-scrypt` and `kdna-password-protected-v1` (Argon2id) are distinct profile IDs. Consumers can support both. Future Studio exports can default to Argon2id while scrypt remains a legacy read-only profile.
4. **ADR-005 compliance.** ADR-005 forbids *new* profiles until existing ones are frozen. The scrypt profile is a *variant* of `kdna-password-protected-v1`, not a new architectural profile. It shares the same envelope structure, key wrapping, AAD rules, and CEK model; only the KDF differs.
...
| Profile ID | `kdna-password-protected-v1-scrypt` |

### Encryption Profile Envelope

Same envelope structure as `kdna-password-protected-v1`:
...
1. v0.1: `kdna-password-protected-v1-scrypt` is the write profile.
2. v0.2: Add `kdna-password-protected-v1` (Argon2id, via `@noble/hashes`) as an additional write profile. Studio export offers `--password --kdf argon2id`.
3. v0.3+: `kdna-password-protected-v1` (Argon2id) becomes the default write profile. `kdna-password-protected-v1-scrypt` becomes **read-only legacy** (decrypt still supported, but new exports use Argon2id).
...
- `kdna-password-protected-v1-scrypt` is added to `crypto-profile.js` (new `encryptProtectedEntryScrypt` / `decryptProtectedEntryScrypt`).
...
- `kdna-core` `v1/index.js` `inferEntitlementProfile()` gains detection of `kdna-password-protected-v1-scrypt`.
- e2e test replaces the "exits 2" stub test with round-trip + fail-closed tests.
- ADR-005's profile inventory should be updated to list `kdna-password-protected-v1-scrypt` as the v0.1 write profile and `kdna-password-protected-v1` (Argon2id) as read-only until v0.2.
