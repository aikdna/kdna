...
> **历史产品概念。** 这不是当前 KDNA Core 组件，也不是已经公开交付的服务。KDNA Core 提供内容中立的资产与运行时契约；治理控制台属于可选的上层应用。下文命令不是当前 CLI 指南。
...
Studio 和 CLI 可以提供创作与运行时原语，Console 可以增加审阅流程；记忆、Outcome、权限、评测和部署仍是独立的应用职责。
