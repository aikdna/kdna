...
The `kdna/registry/` directory in the main KDNA repository is a **deprecated historical artifact**. It was used for early CLI/validator development and does not represent the current architecture. Current KDNA Core uses local-first asset loading without a required registry.
