...
```bash
npm install -g @aikdna/kdna-cli   # 运行时 CLI（0.30.3）
npm install -g @aikdna/kdna-studio-cli  # 创作 CLI（0.8.16）
```
...
AIKDNA 过去发布的示范资产已经归档，因为它们早于当前单一 CBOR 格式。
当前上手路径由 CLI 本地生成示范资产；协议不依赖官方持续生产内容。

→ [历史示范归档](https://github.com/aikdna/kdna-assets)
