...
The KDNA CLI is the runtime control plane. It can `inspect`, `validate`, `pack`, and `unpack` `.kdna` files. The current phase introduces a minimal closed loop on the canonical format.

Implementation: [`aikdna/kdna-cli`](https://github.com/aikdna/kdna-cli)

For the current reference implementation that operates on the canonical format, see `scripts/` in this repo:
