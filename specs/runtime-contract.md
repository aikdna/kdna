...
- [ConsumptionPlan 1.0](./consumption-plan.schema.json)
- [Agent Host capability descriptor 1.0](./agent-host-capabilities.schema.json)
- [Agent Host 2 request](./agent-host-request.schema.json)
- [Agent Host 2 correlated response receipt](./agent-host-receipt.schema.json)
- [JudgmentTrace 1.0](./judgment-trace.schema.json)
...
`judgment-trace.schema.json`. It is not the CLI's historical
...
Profile ID: `kdna.canonicalization.consumption-plan-jcs`
...
`agent-host-capabilities.schema.json` and uses
...
5. Require a `registered_descriptor` that advertises
   `kdna.canonicalization.runtime-capsule-jcs`.
...
2. validate the embedded Capsule against `runtime-capsule.schema.json`;
...
`agent-host-receipt.schema.json`. Its root `protocol` and `request_id` echo
the request. The required `runtime_receipt` records:

- Capsule version `2.0`;
- profile `kdna.canonicalization.runtime-capsule-jcs`;
...
When `result_ref` is non-null, `kdna.canonicalization.result-jcs` means SHA-256 over the
...
`conformance/runtime-contract/`. The verifier performs both JSON Schema and
