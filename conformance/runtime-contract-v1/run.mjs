...
const CORE_VERSIONS = ['2.0', '1.0'];

function readJson(name) {
  return core.parseExecutionContractJsonV1(fs.readFileSync(path.join(HERE, name)));
}
...
function createHostMismatchTrace() {
  ...
  return core.buildJudgmentTraceV1(
    {
...
function validateGoldens() {
  ...
  assert.equal(core.computeConsumptionPlanDigestV1(golden.plan), golden.plan.integrity.plan_digest);

  const builtPlan = core.buildConsumptionPlanV1(buildInputForPlan(golden.plan));
  assert.ok(equalJson(builtPlan, golden.plan), 'ConsumptionPlan builder must reproduce golden');

  const pair = core.negotiateExecutionPairV1(golden.plan, baseContext());
  assert.equal(pair.state, 'selected');

  assertValid('Host 2 request', core.validateAgentHost2RequestV1(golden.request, baseContext()));
  const builtRequest = core.buildAgentHost2RequestV1(
  ...
  assertValid(
    'Host 2 receipt',
    core.validateAgentHost2ReceiptV1(golden.receipt, { request: golden.request }),
  );
  assert.ok(
    equalJson(
  ...
  for (const trace of positiveTraces) {
    ...
    const builtTrace = core.buildJudgmentTraceV1(
      {
  ...
  for (const trace of positiveTraces.filter((value) => value.overall_status !== 'blocked')) {
    ...
    assert.equal(
      failureCode(core.validateJudgmentTraceV1(invalid, contextForTrace(trace))),
      'SCHEMA_INVALID',
...
function validateNegotiationVectors() {
  for (const vector of negotiationVectors.cases) {
    ...
    assert.deepEqual(
      core.negotiateExecutionPairV1(plan, {
...
function validateMutated(base, value) {
  if (base === 'plan') {
    return core.validateConsumptionPlanV1(value, {
      trustedPlanDigest: golden.plan.integrity.plan_digest,
    });
  }
  if (base === 'capabilities') {
    const pair = core.negotiateExecutionPairV1(golden.plan, {
      trustedPlanDigest: golden.plan.integrity.plan_digest,
      capabilities: value,
      coreCapsuleVersions: CORE_VERSIONS,
    });
  ...
  if (base === 'request') return core.validateAgentHost2RequestV1(value, baseContext());
  if (base === 'receipt') {
    return core.validateAgentHost2ReceiptV1(value, { request: golden.request });
  }
  if (base === 'trace') return core.validateJudgmentTraceV1(value, baseContext());
...
function validateAuditNegativeVectors() {
  const scenarios = {
    'plan-accepts-capsule-1'() {
      ...
      return failureCode(
        core.validateConsumptionPlanV1(plan, {
          trustedPlanDigest: golden.plan.integrity.plan_digest,
        }),
      );
    },
    'legacy-descriptor-selected-for-plan-1'() {
      return core.negotiateExecutionPairV1(golden.plan, {
        trustedPlanDigest: golden.plan.integrity.plan_digest,
        coreCapsuleVersions: ['2.0'],
        capabilities: {
    ...
    'request-without-plan-ref'() {
      ...
      return failureCode(core.validateAgentHost2RequestV1(request, baseContext()));
    },
    ...
    'trace-host-descriptor-substitution'() {
      ...
      return failureCode(core.validateJudgmentTraceV1(trace, baseContext()));
    },
    ...
    'p-mismatch-provider-execution'() {
      ...
      return failureCode(
        core.validateJudgmentTraceV1(trace, baseContext({ receipt: trace.host_receipt })),
      );
    },
    'p-mismatch-equal-values'() {
      ...
      return failureCode(core.validateJudgmentTraceV1(trace, baseContext({ receipt })));
    },
  };
...
function validateRawJsonBoundary() {
  assert.deepEqual(core.parseExecutionContractJsonV1('{"outer":{"value":1},"items":[true,null]}'), {
    outer: { value: 1 },
    items: [true, null],
  });
  assert.throws(
    () => core.parseExecutionContractJsonV1('{"a":1,"\\u0061":2}'),
  ...
  const proto = core.parseExecutionContractJsonV1('{"__proto__":{"polluted":true}}');
