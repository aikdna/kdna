...
const planSchema = require('../schema/consumption-plan.schema.json');
const capabilitiesSchema = require('../schema/agent-host-capabilities.schema.json');
const requestSchema = require('../schema/agent-host-request.schema.json');
const receiptSchema = require('../schema/agent-host-receipt.schema.json');
const traceSchema = require('../schema/judgment-trace.schema.json');

const PLAN_DIGEST_PROFILE = 'kdna.canonicalization.consumption-plan-jcs';
const PLAN_DIGEST_PROFILE_VERSION = '0.1.0';
const CAPSULE_DELIVERY_PROFILE = 'kdna.canonicalization.runtime-capsule-jcs';
const CAPSULE_DELIVERY_PROFILE_VERSION = '0.1.0';
const CAPSULE_DIGEST_PROFILE = 'kdna.digest-evidence';
const HOST_PROTOCOL = 'kdna.agent-host';
const HOST_PROTOCOL_VERSION = '0.1.0';
const CAPSULE_CONTRACT_VERSION = '0.1.0';
const DEFAULT_CORE_CAPSULE_CONTRACT_VERSIONS = Object.freeze([
...
class KDNARuntimeContractError extends Error {
  constructor(code, message, details) {
    super(message);
    this.name = 'KDNARuntimeContractError';
    this.code = code;
...
function throwInvalid(result) {
  ...
  throw new KDNARuntimeContractError(
...
function snapshotJsonValue(value) {
  return parseRuntimeContractJson(canonicalizeJcs(value));
}
  ...
  try {
    ...
    if (
      ...
      return invalid(
        ...
        'ConsumptionPlan requires a packaged .kdna Runtime asset.',
        details,
      );
    }
    if (
 ...
 */
function parseRuntimeContractJson(input, options = {}) {
  if (
    options === null ||
...
class StrictJsonParser {
  ...
  syntax(message) {
    throw new KDNARuntimeContractError('KDNA_JSON_SYNTAX_INVALID', message, {
...
function computeConsumptionPlanDigest(plan) {
  try {
    ...
    if (error instanceof KDNARuntimeContractError) throw error;
    throw new KDNARuntimeContractError(
...
function validatePlanSemantics(plan, trustedPlanDigest) {
  let computed;
  try {
    computed = computeConsumptionPlanDigest(plan);
  } catch {
...
function validateConsumptionPlan(plan, context) {
...
function buildConsumptionPlan(input) {
  let plan;
  try {
    plan = {
      ...
      projection_request: {
        ...
        accepted_capsule_versions: [CAPSULE_CONTRACT_VERSION],
      ...
      integrity: {
        profile: PLAN_DIGEST_PROFILE,
        profile_version: PLAN_DIGEST_PROFILE_VERSION,
        plan_digest: '',
      },
      constraints: clone(input.constraints),
    };
    plan.integrity.plan_digest = computeConsumptionPlanDigest(plan);
  } catch (error) {
    if (error instanceof KDNARuntimeContractError) throw error;
    throw new KDNARuntimeContractError(
      'KDNA_PLAN_INPUT_INVALID',
      'Cannot build ConsumptionPlan from the supplied input.',
    );
  }
  const result = validateConsumptionPlan(plan, {
...
function negotiateRuntimePair(plan, context) {
  ...
  const safeContext = contextResult.value;
  const planResult = validateConsumptionPlan(plan, {
  ...
  const hasCapsule =
    safePlan.projection_request.accepted_capsule_versions.includes(CAPSULE_CONTRACT_VERSION) &&
    safeContext.coreCapsuleVersions.includes(CAPSULE_CONTRACT_VERSION) &&
    safeCapabilities.capsule_versions.includes(CAPSULE_CONTRACT_VERSION);
  if (!hasCapsule) return blocked('KDNA_CAPSULE_CONTRACT_VERSION_UNSUPPORTED');
  ...
  return {
    state: 'selected',
    capsule_version: CAPSULE_CONTRACT_VERSION,
...
function expectedAgainst(source) {
  ...
  if (source === 'checksums.json.entry_set_digest') {
...
function validateRequestSemantics(request, plan, enforceBudget = true) {
  if (
    ...
    request.plan_ref.plan_digest_profile_version !== plan.integrity.profile_version ||
    request.plan_ref.plan_digest !== plan.integrity.plan_digest ||
    computeConsumptionPlanDigest(plan) !== plan.integrity.plan_digest
  ) {
  ...
  if (
    request.runtime_contract.capsule_version !== request.capsule.contract_version ||
    request.capsule.contract_version !== CAPSULE_CONTRACT_VERSION
  ) {
    return 'KDNA_HOST_CAPSULE_CONTRACT_VERSION_MISMATCH';
  }
  if (
    request.runtime_contract.capsule_digest_profile !== CAPSULE_DELIVERY_PROFILE ||
    request.runtime_contract.capsule_digest_profile_version !==
      CAPSULE_DELIVERY_PROFILE_VERSION ||
    request.capsule.digests.profile !== plan.projection_request.required_digest_profile ||
    request.capsule.digests.profile_version !==
      plan.projection_request.required_digest_profile_version
  ) {
  ...
  const expectedProjectionContract = {
    ...
    required_digest_profile_version:
      plan.projection_request.required_digest_profile_version,
...
function validateAgentHostRequest(request, context) {
  const contextResult = snapshotContext(context, [
    'plan',
    'trustedPlanDigest',
    'capabilities',
  ...
  const planResult = validateConsumptionPlan(safeContext.plan, {
    trustedPlanDigest: safeContext.trustedPlanDigest,
  });
  if (!planResult.valid) return planResult;
  const safePlan = planResult.value;
  const negotiation = negotiateRuntimePair(safePlan, safeContext);
  if (negotiation.state !== 'selected') {
    return invalid(negotiation.issue_code, 'No strict ConsumptionPlan / Agent Host pair was selected.');
  }
  try {
    const code = validateRequestSemantics(safeRequest, safePlan, true);
    return code ? invalid(code, 'Agent Host request correlation failed.') : ok(safeRequest);
  } catch (error) {
    return invalid('KDNA_INPUT_INVALID', 'Agent Host request is not a safe JSON value.', {
      cause: error && typeof error.message === 'string' ? error.message : String(error),
    });
  }
}

function assembleAgentHostRequest(input, plan) {
  return {
    ...
    plan_ref: {
      ...
      plan_digest_profile_version: plan.integrity.profile_version,
      plan_digest: plan.integrity.plan_digest,
    },
    runtime_contract: {
      capsule_version: CAPSULE_CONTRACT_VERSION,
      capsule_digest_profile: CAPSULE_DELIVERY_PROFILE,
      capsule_digest_profile_version: CAPSULE_DELIVERY_PROFILE_VERSION,
...
function buildAgentHostRequest(input, context) {
  const contextResult = snapshotContext(context, [
    'plan',
    'trustedPlanDigest',
    'capabilities',
  ...
  const negotiation = negotiateRuntimePair(plan, safeContext);
  if (negotiation.state !== 'selected') {
    throw new KDNARuntimeContractError(
      negotiation.issue_code,
      'Cannot build Agent Host request without a selected strict execution pair.',
    );
  }
  let request;
  try {
    request = assembleAgentHostRequest(inputResult.value, plan);
  } catch {
    throw new KDNARuntimeContractError(
      'KDNA_HOST_REQUEST_INPUT_INVALID',
      'Cannot build Agent Host request from the supplied input.',
    );
  }
  const result = validateAgentHostRequest(request, { ...safeContext, plan });
  throwInvalid(result);
  return request;
}
...
function validateAgentHostReceipt(receipt, context) {
  ...
  if (!requestSchemaResult.valid) {
    return invalid(
      'KDNA_VALIDATION_CONTEXT_INVALID',
      'Receipt validation requires a schema-valid independent Agent Host request.',
  ...
  try {
    ...
    return code ? invalid(code, 'Agent Host receipt correlation failed.') : ok(safeReceipt);
  } catch (error) {
    return invalid('KDNA_INPUT_INVALID', 'Agent Host receipt is not a safe JSON value.', {
      cause: error && typeof error.message === 'string' ? error.message : String(error),
    });
  }
}
...
function deriveBudgetEvidence(plan, context) {
  ...
  if (safeContext.request !== null) {
    ...
    if (!requestResult.valid) {
      throw new KDNARuntimeContractError(
        'KDNA_VALIDATION_CONTEXT_INVALID',
        'Budget derivation requires a schema-valid independent Agent Host request.',
    ...
    if (requestCode !== null) {
      throw new KDNARuntimeContractError(
        requestCode,
        'Budget derivation requires an independently correlated Agent Host request.',
      );
    }
  }
  if (safeContext.receipt !== null) {
    ...
    const receiptResult = validateAgentHostReceipt(safeContext.receipt, {
...
function preparePreHostBudgetBlocked(capsule, safeContext) {
  const planResult = validateConsumptionPlan(safeContext.plan, {
    trustedPlanDigest: safeContext.trustedPlanDigest,
  });
  if (!planResult.valid) return planResult;
  ...
  try {
    projectedCandidate = assembleAgentHostRequest(
 ...
 */
function buildPreHostBudgetBlockedTrace(input, context) {
  ...
  const safeContext = contextResult.value;
  const prepared = preparePreHostBudgetBlocked(safeInput.capsule, safeContext);
  throwInvalid(prepared);

  return buildJudgmentTrace(
    {
 ...
 */
function validatePreHostBudgetBlockedTrace(trace, context) {
  const contextResult = snapshotContext(context, [
    'plan',
    'trustedPlanDigest',
    'capabilities',
  ...
  if (!contextResult.valid) return contextResult;
  const safeContext = contextResult.value;
  const prepared = preparePreHostBudgetBlocked(safeContext.capsule, safeContext);
  if (!prepared.valid) return prepared;
  const traceResult = validateJudgmentTrace(trace, {
    ...safeContext,
    plan: prepared.value.plan,
    request: prepared.value.projectedCandidate,
    receipt: null,
...
function unavailableDigestEvidence() {
  ...
  return {
    ...
    asset: item('kdna.digest-basis.container-bytes'),
    content: item('kdna.digest-basis.content-tree'),
    runtime_entry_set: item('kdna.digest-basis.runtime-entry-set'),
  };
}
...
function validateTraceSemantics(
  ...
  if (
    ...
    trace.plan_ref.plan_digest_profile_version !== plan.integrity.profile_version ||
    trace.plan_ref.plan_digest !== plan.integrity.plan_digest ||
    trace.plan_ref.plan_digest !== computeConsumptionPlanDigest(plan)
  ) {
  ...
  if (trace.overall_status === 'blocked') {
    ...
    if (request !== null) {
      ...
      const negotiation = negotiateRuntimePair(plan, {
        trustedPlanDigest: plan.integrity.plan_digest,
        capabilities,
        coreCapsuleVersions,
      });
      ...
      if (delivery.host_boundary_comparison === 'mismatched') {
        if (
          ...
          delivery.delivered_capsule_version !== CAPSULE_CONTRACT_VERSION ||
...
function validateJudgmentTrace(trace, context) {
  const contextResult = snapshotContext(context, [
    'plan',
    'trustedPlanDigest',
    'capabilities',
...
function buildJudgmentTrace(input, context) {
  const contextResult = snapshotContext(context, [
    'plan',
    'trustedPlanDigest',
    'capabilities',
  ...
  if (safeContext.request !== null) {
    ...
    const negotiation = negotiateRuntimePair(safeContext.plan, safeContext);
    if (negotiation.state !== 'selected') {
      throw new KDNARuntimeContractError(
        negotiation.issue_code,
  ...
  try {
    trace = {
      ...
      capsule_delivery_evidence: {
        ...
        basis_version: CAPSULE_DELIVERY_PROFILE_VERSION,
        observed: p,
        ...
        delivered_capsule_version:
          hasReceipt || observation === 'not_observed' ? CAPSULE_CONTRACT_VERSION : null,
    ...
    throw new KDNARuntimeContractError(
      ...
      'Cannot build JudgmentTrace from the supplied input and independent context.',
    );
  }

  const result = validateJudgmentTrace(trace, safeContext);
  throwInvalid(result);
  return trace;
}

module.exports = {
  KDNARuntimeContractError,
  PLAN_DIGEST_PROFILE,
  PLAN_DIGEST_PROFILE_VERSION,
  HOST_PROTOCOL,
  HOST_PROTOCOL_VERSION,
  DEFAULT_CORE_CAPSULE_CONTRACT_VERSIONS,
  parseRuntimeContractJson,
  computeConsumptionPlanDigest,
  buildConsumptionPlan,
  validateConsumptionPlan,
  negotiateRuntimePair,
  buildAgentHostRequest,
  validateAgentHostRequest,
  validateAgentHostReceipt,
  deriveBudgetEvidence,
