...
const golden = core.parseRuntimeContractJson(
  fs.readFileSync(path.join(REPO_ROOT, 'conformance', 'runtime-contract', 'golden.json')),
);
...
test('strict JSON parser matches valid JSON values and preserves protocol number semantics', () => {
  ...
  for (const raw of corpus) {
    assert.deepEqual(core.parseRuntimeContractJson(raw), JSON.parse(raw), raw);
  }
  assert.equal(Object.is(core.parseRuntimeContractJson('-0'), -0), true);
});
...
test('strict JSON parser rejects decoded duplicate keys at every object depth', () => {
  ...
  for (const raw of duplicates) assertCode('KDNA_JSON_DUPLICATE_KEY', () => core.parseRuntimeContractJson(raw));
  assert.deepEqual(core.parseRuntimeContractJson('{"a":1,"nested":{"a":2}}'), {
    a: 1,
    nested: { a: 2 },
  });
  assert.deepEqual(core.parseRuntimeContractJson('{"é":1,"é":2}'), { é: 1, é: 2 });
});

test('strict JSON parser preserves hostile names as own data properties', () => {
  const parsed = core.parseRuntimeContractJson(
...
test('strict JSON parser enforces UTF-8, BOM, Unicode scalar, grammar, and finite numbers', () => {
  ...
  for (const bytes of invalidUtf8) {
    assertCode('KDNA_JSON_INVALID_UNICODE', () =>
      core.parseRuntimeContractJson(Uint8Array.from(bytes)),
    );
  }
  assertCode('KDNA_JSON_BOM_FORBIDDEN', () =>
    core.parseRuntimeContractJson(Buffer.from([0xef, 0xbb, 0xbf, 0x7b, 0x7d])),
  );
  assertCode('KDNA_JSON_BOM_FORBIDDEN', () => core.parseRuntimeContractJson('\ufeff{}'));

  for (const raw of ['"\ud800"', '"\udc00"', '"\\ud800"', '"\\udc00"']) {
    assertCode('KDNA_JSON_INVALID_UNICODE', () => core.parseRuntimeContractJson(raw));
  }
  for (const code of [0x00, 0x01, 0x1f]) {
    assertCode('KDNA_JSON_SYNTAX_INVALID', () =>
      core.parseRuntimeContractJson(`"${String.fromCharCode(code)}"`),
    );
  }
  for (const raw of [
    '',
    ...
    assertCode('KDNA_JSON_SYNTAX_INVALID', () => core.parseRuntimeContractJson(raw));
  }
  assertCode('KDNA_JSON_NUMBER_INVALID', () => core.parseRuntimeContractJson('1e400'));
  assert.deepEqual(core.parseRuntimeContractJson('"\\u0000\\u001f"'), '\u0000\u001f');

  const surrounded = Buffer.from('xx{"a":1}yy');
  assert.deepEqual(core.parseRuntimeContractJson(new Uint8Array(surrounded.buffer, surrounded.byteOffset + 2, 7)), { a: 1 });
});
...
test('strict JSON parser limits bytes and container depth and only allows tightening options', () => {
  ...
  assert.equal(core.parseRuntimeContractJson(raw, { maxBytes: 8 }), '你好');
  assertCode('KDNA_JSON_LIMIT_EXCEEDED', () =>
    core.parseRuntimeContractJson(raw, { maxBytes: 7 }),
  );
  assert.deepEqual(core.parseRuntimeContractJson('[1]', { maxDepth: 1 }), [1]);
  assertCode('KDNA_JSON_LIMIT_EXCEEDED', () =>
    core.parseRuntimeContractJson('[[]]', { maxDepth: 1 }),
  );
  assert.deepEqual(core.parseRuntimeContractJson('[[]]', { maxDepth: 2 }), [[]]);
  assertCode('KDNA_JSON_LIMIT_EXCEEDED', () =>
    core.parseRuntimeContractJson('[[[]]]', { maxDepth: 2 }),
  );
  ...
  for (const options of [
    ...
    assertCode('KDNA_JSON_OPTIONS_INVALID', () => core.parseRuntimeContractJson('{}', options));
  }
  const accessor = {};
  Object.defineProperty(accessor, 'maxDepth', { get() { throw new Error('must not run'); } });
  assertCode('KDNA_JSON_OPTIONS_INVALID', () => core.parseRuntimeContractJson('{}', accessor));
});
...
test('builders reproduce goldens without mutating independent inputs', () => {
  ...
  const plan = core.buildConsumptionPlan(input);
...
test('every stable ConsumptionPlan chain requires a non-null matching trusted Plan digest', () => {
  ...
  for (const [trustedPlanDigest, expectedCode] of [
    ...
    assert.equal(
      core.validateConsumptionPlan(golden.plan, { trustedPlanDigest }).code,
      expectedCode,
    );
    assert.equal(
      core.negotiateRuntimePair(golden.plan, {
        trustedPlanDigest,
        capabilities: golden.capabilities,
        coreCapsuleVersions: ['0.1.0'],
      }).issue_code,
    ...
    assert.equal(
      core.validateJudgmentTrace(golden.trace, context({ trustedPlanDigest })).code,
      expectedCode,
    );
...
test('public Agent Host request validation cannot disable pre-Host budget enforcement', () => {
  ...
  const result = core.validateAgentHostRequest(
    request,
    context({
      plan,
...
test('pre-Host budget block preserves projected A/C/E/P and exact exceeded evidence without a request', () => {
  ...
  assert.equal(
    core.validatePreHostBudgetBlockedTrace(trace, {
      ...executionContext,
      capsule,
    }).valid,
    true,
  ...
  for (const [label, mutate] of tamperCases) {
    ...
    mutate(tampered);
    assert.equal(
      core.validatePreHostBudgetBlockedTrace(tampered, {
        ...executionContext,
        capsule,
      }).valid,
      false,
...
test('pre-Host budget block supports task overflow but cannot be used within limits', () => {
  ...
  assert.equal(
    core.validatePreHostBudgetBlockedTrace(golden.trace, {
      ...context(),
      capsule,
    }).code,
  ...
  assert.equal(
    core.validatePreHostBudgetBlockedTrace(forged, {
      ...executionContext,
      capsule,
    }).code,
...
test('pre-Host budget block rejects mismatched Capsules and hostile builder inputs', () => {
  ...
  assertCode('KDNA_INPUT_INVALID', () =>
    core.buildPreHostBudgetBlockedTrace(hostileInput, executionContext),
  );
...
test('matched Host receipt requires sender, Host, echo, and actual Capsule P to agree', () => {
  ...
  assert.equal(
    core.validateAgentHostReceipt(receipt, { request }).code,
...
test('Trace builder distinguishes explicit not-delivered and not-observed evidence', () => {
  ...
  const notDelivered = core.buildJudgmentTrace(
    {
  ...
  assert.equal(core.validateJudgmentTrace(notDelivered, notDeliveredContext).valid, true);

  const notObservedContext = context({
    receipt: null,
    trustedDeliveryObservation: 'not_observed',
  });
  const notObserved = core.buildJudgmentTrace(
    {
  ...
  assert.equal(notObserved.capsule_delivery_evidence.delivered_capsule_version, '0.1.0');
  ...
  assert.equal(core.validateJudgmentTrace(notObserved, notObservedContext).valid, true);
});
...
test('object validators snapshot context and reject getters or custom prototypes', () => {
  ...
  assert.equal(
    core.validateConsumptionPlan(golden.plan, planContext).code,
    'KDNA_VALIDATION_CONTEXT_INVALID',
  );

  const requestContext = Object.assign(Object.create({ hostile: true }), context());
  assert.equal(
    core.validateAgentHostRequest(golden.request, requestContext).code,
    'KDNA_VALIDATION_CONTEXT_INVALID',
  );
  ...
  assert.equal(
    core.validateAgentHostReceipt(golden.receipt, receiptContext).code,
    'KDNA_VALIDATION_CONTEXT_INVALID',
  );
  ...
  Object.defineProperty(hostileCapabilities, 'host_protocols', {
    ...
    get() {
      getterRuns += 1;
      return ['kdna.agent-host.unsupported'];
    },
  });
  traceContext.capabilities = hostileCapabilities;
  assert.equal(
    core.validateJudgmentTrace(golden.trace, traceContext).code,
    'KDNA_VALIDATION_CONTEXT_INVALID',
  );
  assert.equal(getterRuns, 0);
});
...
test('object validators fail closed with stable codes instead of leaking TypeError', () => {
  ...
  for (const value of values) {
    const results = [
      core.validateConsumptionPlan(value, { trustedPlanDigest: null }),
      core.validateAgentHostRequest(value, context()),
      core.validateAgentHostReceipt(value, { request: golden.request }),
      core.validateJudgmentTrace(value, context()),
    ];
...
test('all stable runtime contract exports are symmetric across CJS, ESM, and declarations', async () => {
  const esm = await import('../src/index.mjs');
  const exports = {
    KDNARuntimeContractError: 'function',
    PLAN_DIGEST_PROFILE: 'string',
    PLAN_DIGEST_PROFILE_VERSION: 'string',
    HOST_PROTOCOL: 'string',
    HOST_PROTOCOL_VERSION: 'string',
    DEFAULT_CORE_CAPSULE_CONTRACT_VERSIONS: 'object',
    parseRuntimeContractJson: 'function',
    computeConsumptionPlanDigest: 'function',
    buildConsumptionPlan: 'function',
    validateConsumptionPlan: 'function',
    negotiateRuntimePair: 'function',
    buildAgentHostRequest: 'function',
    validateAgentHostRequest: 'function',
    validateAgentHostReceipt: 'function',
    deriveBudgetEvidence: 'function',
    buildPreHostBudgetBlockedTrace: 'function',
    validatePreHostBudgetBlockedTrace: 'function',
    buildJudgmentTrace: 'function',
    validateJudgmentTrace: 'function',
  };
  const declarations = fs.readFileSync(path.join(PACKAGE_ROOT, 'src', 'types.d.ts'), 'utf8');
  assert.equal(Object.keys(exports).length, 19);
  ...
  assert.equal(files.has('src/runtime-contract.js'), true);
  for (const schema of [
  ...
  try {
    fs.writeFileSync(
      ...
      [
        ...
        'declare const executionContext: KDNARuntimePairContext;',
        'declare const hostContext: KDNAAgentHostValidationContext;',
        'declare const capsule: KDNARuntimeCapsule;',
        'const result = validateConsumptionPlan(plan, { trustedPlanDigest: plan.integrity.plan_digest });',
        "const blocked = buildPreHostBudgetBlockedTrace({ trace_id: 'trace_0123456789abcdef', timestamp: '2026-07-15T00:00:00.000Z', capsule }, hostContext);",
        'const blockedValidation = validatePreHostBudgetBlockedTrace(blocked, { ...hostContext, capsule });',
        '// @ts-expect-error caller errors cannot replace the canonical budget error',
        "buildPreHostBudgetBlockedTrace({ trace_id: 'trace_0123456789abcdef', timestamp: '2026-07-15T00:00:00.000Z', capsule, errors: [] }, hostContext);",
        'const selected: "0.1.0" | null = trace.runtime_contract.selected_capsule_version;',
