...
function representativeTypesSource(importSpecifier) {
  return [
    `import { ALG, EXTERNAL_AAD_PROFILE, WORK_PACK_SCHEMA, KDNAManifest, KDNAWorkPackManifest, encryptLicensedEntry, decryptLicensedEntry, encryptProtectedEntry, validateWorkPackManifest, inspectWorkPack, externalEnvelopeAad } from ${JSON.stringify(importSpecifier)};`,
    ...
    'console.log(algorithm, aadProfile, plaintext, protectedEnvelope, kdfName, validation, summary, aad, schemaTitle);',
  ].join('\n');
}
