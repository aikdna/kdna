...
export interface ComparisonCondition {
  ...
  model: string;
  modelConfig?: { temperature?: number; top_p?: number };
  score: number;
...
export type FidelityWarningType =
  | 'calibration_failed'
  | 'high_variance'
  | 'low_task_count'
  | 'evaluator_bias_risk'
...
export interface FidelityResult {
  ...
  targetArtifact: {
    ...
    generator?: { engine: string; version: string };
  };
  sourceKdna: Array<{ name: string; version: string; role: string }>;
  overallScore: number;
  ...
  calibration?: CalibrationAnchors & { calibrationValid: boolean };
  crossModel?: Array<{ model: string; score: number }>;
  ...
  traceRefs?: Array<{ traceId: string; traceType?: string }>;
  warnings?: FidelityWarning[];
  completedAt: string;
  measurementDurationMs?: number;
  evaluator?: { engine: string; version: string };
}
