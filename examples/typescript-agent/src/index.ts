...
function main(): void {
  ...
  const agent = new KDNAAgent("../../examples/decision_state");
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    console.log(`Missing: ${result.missing_elements.length > 0 ? result.missing_elements.join(", ") : "None"}`);
    console.log(`Misunderstandings: ${result.misunderstandings_detected.length > 0 ? result.misunderstandings_detected.join("; ") : "None detected"}`);
