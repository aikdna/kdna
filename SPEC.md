...
KDNA source workspaces may be edited manually during development, but trusted
`.kdna` assets MUST be generated through KDNA Studio Core or another
Studio-compatible compiler. Manual JSON editing can produce non-canonical dev
source only; it is not a trusted asset creation path.

### 15.1 Studio-Generated Domains

Assets generated by Studio Core MUST:

1. Conform to all structural requirements in this specification
2. Include the standard `meta` object in every KDNA JSON file
3. Pass schema validation and cross-file validation
...
6. Include authoring provenance, asset identity, `content_digest`, and compiler
   metadata in `kdna.json`
7. Emit the required build reports defined by
   [Studio Export Contract](./docs/STUDIO_EXPORT_CONTRACT.md)
