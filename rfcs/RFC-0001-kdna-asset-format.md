...
- A conforming `.kdna` asset MAY NOT be created by directly packaging arbitrary
  source directories and presenting the result as release-reviewed.
- A release-reviewed `.kdna` asset MUST be compiled by a KDNA-compatible
  authoring pipeline that records provenance, compiler metadata, and asset
  digest. Human Lock evidence is required only when the asset claims human
  confirmation.
- Dev source directories MUST NOT be treated as installable or publishable
  runtime assets unless compiled by a Studio-compatible compiler.
