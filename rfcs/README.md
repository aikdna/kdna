...
- [RFC-0021: KDNA Signature Track](./RFC-0021-signature-track.md) — draft signature track turning RFC-0006's long-term signing model into an independently verifiable wire contract. Milestone M1 (payload canonicalization, `kdsig.ed25519`) is implemented in the JavaScript and Python Cores and bound as a pre-release candidate; M2–M6 (identity, bundle format, rotation/revocation, offline verification, 1.0 compatibility) remain non-binding until their own implementations land, under the fail-closed / non-collapse / provenance-is-not-correctness / offline invariants.
