...
function runDemo() {
  ...
  console.log("\n=== What Changed? ===\n");
  console.log("Without KDNA, the AI summarizes content — producing a pleasant but useless summary of a non-decision.");
  console.log("With KDNA, the AI checks structural elements (owner, timing, choice, action) and classifies the state.");
  console.log();
  console.log("Key differences:");
  console.log("  1. Generic AI: 'No immediate action required' — misses that NO action was ever committed.");
  console.log("  2. KDNA: Explicitly identifies 4 missing elements that prevent execution.");
  console.log("  3. KDNA: Distinguishes vague deferral from intentional deferral using the Deferral Validation framework.");
  console.log("  4. KDNA: Shows which axioms and frameworks were triggered — transparent judgment path.");
  console.log();
  ...
  try {
    ...
    if (domain) {
      ...
      console.log("    → Scenario shows 'discussed' and 'after debate' but no commitment structure.");
      ...
      console.log("  ✗ Did I verify deferral language has specific condition, date, and decision owner?");
...
function buildTrace() {
  ...
  return {
    ...
    triggered_frameworks: [
      {
        ...
        steps_applied: ["Check condition specificity", "Check deferral date", "Check decision owner"]
      }
    ],
    ...
    self_checks: [
      ...
      {
        check_id: "SC-5",
        description: "Did I verify deferral language has specific condition, date, and decision owner?",
        passed: false,
    ...
    generated_judgment: {
      ...
      recommended_action: "Before execution: assign owner, set date, define 'enough data' criteria. Do not treat as decided.",
      reasoning_summary: "Input shows discussion and vague deferral without operational commitment structure. All four commitment elements are missing."
    },
    agent_info: {
