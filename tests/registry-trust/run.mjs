...
const testHome = fs.mkdtempSync(path.join(os.tmpdir(), 'kdna-registry-trust-home-'));
const testKdnaHome = path.join(testHome, '.kdna');
fs.mkdirSync(fixturesDir, { recursive: true });
fs.mkdirSync(registryDir, { recursive: true });
fs.mkdirSync(testKdnaHome, { recursive: true });

function runKdna(command, options = {}) {
...
function makeZip(entries) {
  ...
  return Buffer.concat([
    ...
    Buffer.concat([
      ...
      u16(centralParts.length),
      u32(Buffer.concat(centralParts).length),
      u32(Buffer.concat(localParts).length),
      u16(0),
    ]),
  ]);
}
...
const validAssetZip = makeZip({
  ...
  'KDNA_Patterns.json': json({
    ...
    misunderstandings: [],
    self_check: ['Is this a valid E2E test?'],
  }),
});
...
try {
  runKdna(`kdna registry refresh`, {
    env: { KDNA_REGISTRY_URL: `file://${yankedPath}` },
  });
  runKdna(`kdna install @trust-e2e/minimal`, {
    timeout: 15000,
    env: { KDNA_REGISTRY_URL: `file://${yankedPath}` },
  });
...
try {
  ...
  runKdna(`kdna install @untrusted-e2e/no-trust`, {
    timeout: 15000,
    env: { KDNA_REGISTRY_URL: `file://${noTrustPath}` },
  });
  ...
  assert.ok(
    code !== 0,
    `v1.0-rc: scoped domain without trust_pubkey must be rejected (got exit ${code})`,
  );
...
try {
  runKdna(`kdna verify ${tamperedPath}`);
...
const badMimeAsset = makeZip({
  ...
  'KDNA_Core.json': json({
    ...
    stances: [],
    axioms: [{ id: 'ax_0', one_sentence: 'T.', full_statement: 'Test.', why: 'T.' }],
    ontology: [],
    frameworks: [],
    core_structure: [],
  }),
...
try {
  runKdna(`kdna verify ${badMimePath} --json`);
