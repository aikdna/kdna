...
// a symlinked or aliased directory still RUNS the generation (and can never exit
...
if (entryGuard === 'unresolved-entry') {
  console.error(
    'KDNA_POST_CUTOVER_TOKEN_AUTHORITY_ENTRY_GUARD_FAILED: refusing to run under an unresolved entry path',
  );
  process.exit(2);
}
if (entryGuard === 'entry') main();
