...
if(process.argv[1]&&path.resolve(process.argv[1])===SELF){try{console.log(JSON.stringify(generate(process.argv.slice(2))));}catch(e){console.error(JSON.stringify({status:'ERROR',code:e.code??'GENERATION_ERROR',message:e.message}));process.exitCode=1;}}
